"""
The Neural Architecture for Mnist is able to reach around 0.9 Accuracy after 1000 steps.
The Neural Architecutre for Cifar10 is simplified and is to be used as a proof of concept; after 1000 Steps, this CNN should reach around 0.4 Accuracy in classifying among the 10 classes in Cifar10.
A greater number of steps, as well as a more complex neural architecture is needed to increase the Accuracy on Cifar10 so as to be good enough for practical uses.
"""
...
def mnist_model_fn(features, labels, mode):
    ...
    # Get the Data from the argument 'features' and shape it in [BATCH_SIZE, 28, 28, 1] Size Array
    input_layer = tf.reshape(features["x"], [-1, 28, 28, 1])  # The last '1' in the Size refers to the Number of Channels in the Input Image, RGB will have 3 Channel. But here, there is only 1 Channel.

    # Create a Convolutional Layer; Note that 'padding=same' means that the Height and Width of the Output from Convolutional Layer will be Same as the Input to the Convolutional Layer i.e. the Convolution will not cause the Shrinking of the Input Image due to Zero Padding at the Borders of the Input Image.    
    # Output Image of this Convolutional Layer will be [BATCH_SIZE, 28, 28, 32]. 32 is the Number of Filers
    conv1 = tf.layers.conv2d(
        inputs=input_layer,
        filters=32,
        kernel_size=[5, 5],
    ...
    # Output Image of this Convolutional Layer will be [BATCH_SIZE, 14, 14, 64]. 64 is the number of filters.
    conv2 = tf.layers.conv2d(
        inputs=conv1,
        filters=64,
        kernel_size=[5, 5],
        padding="same",
        activation=tf.nn.relu)  # Each Filter has the Size of [5, 5, 32]. 32 is the Depth of the Input Image. But because there is 64 Filters, the Output Image will have a Depth of 64.


    # Flatten/Reshape the Pooling Layer so as to prepare for the Next, Fully Connected Layer.
    conv2_flat = tf.reshape(conv2, [-1, 28 * 28 * 64])  # The Reshaped Image will be of the Size [BATCH_SIZE, 3136]

    # Create a Fully Connected Layer that has 1024 Neurons/Nodes. And it uses the ReLU Activation Function.
    # Output Image of this Fully Connected Layer will be of the Size [BATCH_SIZE, 1024]
...
def main(unused_argv):
    ...
    choice = input('Enter \'0\' for using Mnist, \'1\' for using Cifar10: ')    
    choice = int(choice)
    print("Processing...")
    ...
    if choice:
        ...
        # There are 6 batches/parts in the Cifar10 Dataset
        ...
        data_batch_3_data = data_batch_3[b'data']   
        data_batch_3_data = np.reshape(data_batch_3_data,(10000,1024,3), 'F') 
        data_batch_3_labels = data_batch_3[b'labels']

        data_batch_4_dir = data_dir + '/data_batch_4' 
        data_batch_4 = unpickle(data_batch_4_dir)
        data_batch_4_data = data_batch_4[b'data']   
        data_batch_4_data = np.reshape(data_batch_4_data,(10000,1024,3), 'F') 
        data_batch_4_labels = data_batch_4[b'labels']


        data_batch_5_dir = data_dir + '/data_batch_5' 
        data_batch_5 = unpickle(data_batch_5_dir)
        data_batch_5_data = data_batch_5[b'data']   
        data_batch_5_data = np.reshape(data_batch_5_data,(10000,1024,3), 'F') 
        ...
        # Create the Estimator using the 'cifar10_model_fn' that we created way above. [To create an Estimator, we need to first build the model function 'cifar10_model_fn]
        ...
        model_fn=cifar10_model_fn, model_dir="/tmp/cifar10_convnet_model") # model_dir can be any directory that you like.
        ...
        # Get a series of numpy arrays [4 arrays in total]:
        train_data = mnist.train.images  # Training Set
        train_labels = np.asarray(mnist.train.labels, dtype=np.int32) # Labels for the Training Set
        eval_data = mnist.test.images  # Returns Evaluation Set
        eval_labels = np.asarray(mnist.test.labels, dtype=np.int32) # Labels for the Evaluation Set

        # From the Evaluation Set, get 3 first Elements to form our Test Set and its Labels
        test_data = eval_data[0:3]
        test_label = eval_labels[0:3]

        # Create the Estimator using the 'cnn_model_fn' that we created way above. [To create an Estimator, we need to first build the model function 'mnist_model_fn]
        classifier = tf.estimator.Estimator(
        model_fn=mnist_model_fn, model_dir="/tmp/mnist_convnet_model") # model_dir can be any directory that you like.
  ...
  # Log the values in the "Softmax" tensor using the label "probabilities"
    ...
    # Now, we call the 'train' method of our Estimator [classifier]. Notice that we added in the Logging Hook as an argument.
    classifier.train(
    ...
    # Now, we call the 'evaluate' method of our Estimator [classifier]
    ...
    # Now, we will call the 'predict' method of our Estimator [classifier]
